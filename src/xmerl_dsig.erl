...
verify(Element, Fingerprints) ->
    ...
    CanonXml = xmerl_c14n:c14n(strip(Element), false, InclNs),
    CanonXmlUtf8 = unicode:characters_to_binary(CanonXml, unicode, utf8),
    CanonSha64 = crypto:hash(HashFunction, CanonXmlUtf8),

    [#xmlText{value = DigestSha64}] = xmerl_xpath:string("ds:Signature/ds:SignedInfo/ds:Reference/ds:DigestValue/text()", Element, [{namespace, DsNs}]),
    DigestCanonSha64 = esaml_util:base64_decode(DigestSha64),
    ...
    case CanonSha64 of
        DigestCanonSha64 ->
            ...
            CertElements = xmerl_xpath:string("ds:Signature//ds:X509Certificate/text()", Element, [{namespace, DsNs}]),
            verify_signature(Data, HashFunction, Sig, Fingerprints, CertElements);
        _ ->
            {error, {bad_digest, {received, DigestSha64}, {calculated, base64:encode(CanonSha64)}}}
    end.

%% @private
-spec verify_signature(string(), sha| sha256, binary(), [fingerprint()] | any, [#xmlText{}]) ->
    ok |  {error, cert_not_accepted}| {error, bad_signature}.
verify_signature(Data, HashFunction, Signature, Fingerprints, CertElements) ->
    lists:foldl(fun
        (_, ok) ->
            ok;
        (#xmlText{value = Cert64}, _) ->
            ...
            case public_key:verify(Data, HashFunction, Signature, Key) of
                true ->
    ...
    end, {error, bad_signature}, CertElements).
...
verify_valid_sha1_test() ->
    ...
    ok = verify(Doc),
    ok = verify(Doc, [<<123, 124, 223, 187, 5, 19, 129, 102, 107, 255, 146, 216, 34, 142, 245, 149, 9, 38, 36, 74, 242, 102, 98, 106, 78, 207, 159, 43, 178, 93, 144, 30>>]).
...
z+CClhkG4RiRcwuxMtkA9m0GmjEYh7TeDZJ3ntXaexH3s+IKFwEq2BsF</ds:X509Certificate></ds:X509Data></ds:KeyInfo></ds:Signature><saml2:Subject xmlns:saml2=\"urn:oasis:names:tc:SAML:2.0:assertion\"><saml2:NameID Format=\"urn:oasis:names:tc:SAML:1.1:nameid-format:unspecified\">yaroslav@kato.im</saml2:NameID><saml2:SubjectConfirmation Method=\"urn:oasis:names:tc:SAML:2.0:cm:bearer\"><saml2:SubjectConfirmationData NotOnOrAfter=\"2015-01-09T02:02:56.021Z\" Recipient=\"https://api.kato.im/saml/v2/demo-okta/consume\"/></saml2:SubjectConfirmation></saml2:Subject><saml2:Conditions NotBefore=\"2015-01-09T01:52:56.021Z\" NotOnOrAfter=\"2015-01-09T02:02:56.021Z\" xmlns:saml2=\"urn:oasis:names:tc:SAML:2.0:assertion\"><saml2:AudienceRestriction><saml2:Audience>https://api.kato.im/saml/v2/demo-okta/metadata</saml2:Audience></saml2:AudienceRestriction></saml2:Conditions><saml2:AuthnStatement AuthnInstant=\"2015-01-09T01:57:56.021Z\" SessionIndex=\"id1420768676021.697992970\" xmlns:saml2=\"urn:oasis:names:tc:SAML:2.0:assertion\"><saml2:AuthnContext><saml2:AuthnContextClassRef>urn:oasis:names:tc:SAML:2.0:ac:classes:PasswordProtectedTransport</saml2:AuthnContextClassRef></saml2:AuthnContext></saml2:AuthnStatement></saml2:Assertion></saml2p:Response>", [{namespace_conformant, true}]),
    ok = verify(Doc),
    ok = verify(Doc, [<<194, 197, 153, 135, 143, 219, 201, 180, 191, 197, 158, 136, 28, 149, 34, 86, 70, 92, 129, 233, 67, 29, 32, 225, 176, 162, 240, 143, 126, 109, 244, 209>>]).
...
verify_bad_digest_test() ->
    ...
    ?assertMatch({error, {bad_digest, {received, _}, {calculated, _}}}, verify(Doc)).
...
foreach_key(Fun) ->
    Fun(test_sign_key()),
    Fun(test_sign_256_key()).

sign_and_verify_test() ->
    foreach_key(fun({Key, CertBin}) ->
...
% SAML Signature may contain more than one certificate in the KeyInfo->X509Data, although only one of
% them is used to create the signature. This is used e.g. for cert rollover.
% In such case, all certificates should be checked by the verifier.
sign_and_verify_multiple_signing_certs_test() ->
    {KeyAlpha, CertBinAlpha} = test_sign_256_key(),
    {KeyBeta, CertBinBeta} = test_sign_key(),
    sign_and_verify_multiple_signing_certs_test_base(KeyAlpha, CertBinAlpha, [CertBinAlpha, CertBinBeta]),
    sign_and_verify_multiple_signing_certs_test_base(KeyAlpha, CertBinAlpha, [CertBinBeta, CertBinAlpha]),
    sign_and_verify_multiple_signing_certs_test_base(KeyBeta, CertBinBeta, [CertBinAlpha, CertBinBeta]),
    sign_and_verify_multiple_signing_certs_test_base(KeyBeta, CertBinBeta, [CertBinBeta, CertBinAlpha]).
